        ...
        <div className="mt-8 grid grid-cols-2 gap-2.5 sm:mt-14 sm:gap-3 lg:grid-cols-3 lg:gap-4">
          {areas.map((a, i) => (
            <Reveal key={a.title} index={i % 3} className={i === areas.length - 1 ? "col-span-2 h-full lg:col-span-1" : "h-full"}>
              <div className="flex h-full flex-col gap-3 rounded-[1.25rem] bg-white p-4 shadow-[var(--shadow-card)] ring-hairline sm:rounded-3xl sm:p-6">
                ...
                <div>
                  ...
                  <div className="mt-2.5 flex flex-wrap gap-1 sm:mt-3 sm:gap-1.5">
                    {a.items.map((it) => (
                      <span key={it} className="rounded-full bg-primary/[0.07] px-2 py-0.5 text-[0.65rem] font-medium text-primary sm:px-2.5 sm:py-1 sm:text-[0.72rem]">
                        {it}
                      </span>
                    ))}
                  </div>
